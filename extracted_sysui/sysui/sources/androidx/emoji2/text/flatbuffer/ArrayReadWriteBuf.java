...
public class ArrayReadWriteBuf implements ReadWriteBuf {
    ...
    public int getInt(int i2) {
        ...
        return (bArr[i2] & TransitionInfo.INIT) | (bArr[i2 + 3] << 24) | ((bArr[i2 + 2] & TransitionInfo.INIT) << 16) | ((bArr[i2 + 1] & TransitionInfo.INIT) << 8);
    }
    ...
    public long getLong(int i2) {
        ...
        return (((long) bArr[i2 + 7]) << 56) | j2 | ((((long) bArr[i2 + 6]) & 255) << 48);
    }

    @Override // androidx.emoji2.text.flatbuffer.ReadBuf
    public short getShort(int i2) {
        byte[] bArr = this.buffer;
        return (short) ((bArr[i2] & TransitionInfo.INIT) | (bArr[i2 + 1] << 8));
    }
