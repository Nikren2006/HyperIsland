...
public class FileBinder extends VariableBinder {
    ...
    public static class Variable extends VariableBinder.Variable {
        ...
        public Variable(Element element, Variables variables) {
            ...
            Expression expressionBuild = Expression.build(variables, element.getAttribute(QsFlipEventsKt.EVENT_KEY_FLIP_QS_INDEX));
            this.mIndex = expressionBuild;
            if (expressionBuild == null) {
                MamlLog.e(VariableBinder.Variable.TAG_NAME, "fail to load file index expression");
            }
        }
    ...
    private void load(Element element) {
        if (element == null) {
            MamlLog.e("FileBinder", "FileBinder node is null");
            return;
        }
        String strTrim = element.getAttribute("filter").trim();
        this.mFilters = TextUtils.isEmpty(strTrim) ? null : strTrim.split(aa.f3429b);
    ...
    public void refresh() {
        ...
        MamlLog.i("FileBinder", "file count: " + length);
        updateVariables();
    }
