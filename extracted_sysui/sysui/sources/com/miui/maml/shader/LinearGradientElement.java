...
public class LinearGradientElement extends ShaderElement {
    ...
    public boolean updateShaderMatrix() {
        ...
        float endX = getEndX();
        float endY = getEndY();
        if (x2 == this.mX && y2 == this.mY && endX == this.mEndX && endY == this.mEndY) {
            return false;
        }
        this.mX = x2;
        this.mY = y2;
        ...
        this.mShaderMatrix.setPolyToPoly(new float[]{0.0f, 0.0f, 1.0f, 1.0f}, 0, new float[]{x2, y2, endX, endY}, 0, 2);
        return true;
    }
}
