    ...
    public static ThreadPoolExecutor getLocalTaskExecutor() {
        if (sLocalExecutor == null) {
            synchronized (MUTEX_LOCK) {
                try {
                    if (sLocalExecutor == null) {
                        ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(2, 10, 60L, TimeUnit.SECONDS, new SynchronousQueue(), new ThreadFactory() { // from class: com.miui.maml.util.b
                        ...
                        threadPoolExecutor.setRejectedExecutionHandler(new RejectedExecutionHandler() { // from class: com.miui.maml.util.c
    ...
    public static ThreadPoolExecutor getLongOnlineTaskExecutor() {
        if (sBigOnlineTaskExecutor == null) {
            synchronized (MUTEX_LOCK) {
                try {
                    if (sBigOnlineTaskExecutor == null) {
                        ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(5, 5, 60L, TimeUnit.SECONDS, new LinkedBlockingQueue(), new ThreadFactory() { // from class: com.miui.maml.util.a
                            @Override // java.util.concurrent.ThreadFactory
                            public final Thread newThread(Runnable runnable) {
