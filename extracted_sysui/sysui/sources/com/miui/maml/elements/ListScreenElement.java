...
public class ListScreenElement extends ElementGroup {
    ...
    /* JADX INFO: renamed from: com.miui.maml.elements.ListScreenElement$1, reason: invalid class name */
    ...
    public static class Column {
        ...
        private void load(Element element) {
            ...
            this.mTarget = element.getAttribute(TypedValues.AttributesType.S_TARGET);
        ...
        public void init() {
            if (this.mTargetElement == null) {
                ...
                if (!(screenElementFindElement instanceof VariableArrayElement)) {
                    MamlLog.e(ListScreenElement.LOG_TAG, "can't find VarArray:" + this.mTarget);
                    return;
                }
    ...
    public static class ColumnInfo {
        ...
        public ColumnInfo(String str) {
            int iIndexOf = str.indexOf(MethodCodeHelper.IDENTITY_INFO_SEPARATOR);
            if (iIndexOf == -1) {
    ...
    public ListScreenElement(Element element, ScreenElementRoot screenElementRoot) {
        ...
        if (arrayListCreateColumnsInfo == null) {
            MamlLog.e(LOG_TAG, "invalid item data");
        ...
        if (child == null) {
            MamlLog.e(LOG_TAG, "no attr data binder");
    ...
    public void addColumn(String str, Object[] objArr) {
        ...
        synchronized (this.mLock) {
            try {
                ...
                while (i2 < size2) {
                    this.mDataList.get(i2).setData(i3, i2 < length ? objArr[i2] : null);
    ...
    private void clearEmptyRowLocked() {
        for (int size = this.mDataList.size() - 1; size >= 0; size--) {
            for (Object obj : this.mDataList.get(size).mData) {
                if (obj != null) {
                    return;
                }
            }
    ...
    private void removeItemLocked(int i2) {
        ...
        if (size > 0) {
            Integer numRemove = this.mIndexOrder.remove(i3);
            numRemove.intValue();
            moveTo(this.mInnerGroup.getY());
            this.mReuseIndex.add(numRemove);
        }
        requestUpdate();
    }
    ...
    private void updateScorllBar() {
        ...
        double y2 = ((double) this.mInnerGroup.getY()) / (height2 - height);
        if (y2 > 1.0d) {
            y2 = 1.0d;
        }
        this.mScrollBar.setY((float) ((1.0d - d2) * height2 * y2));
    ...
    public void addItem(Object... objArr) {
        ...
        if (objArr.length != this.mColumnsInfo.size()) {
            MamlLog.e(LOG_TAG, "invalid item data count");
            return;
        }
        int length = objArr.length;
        for (int i2 = 0; i2 < length; i2++) {
            if (!this.mColumnsInfo.get(i2).validate(objArr[i2])) {
                MamlLog.e(LOG_TAG, "invalid item data type: " + objArr[i2]);
                return;
            }
        }
        synchronized (this.mLock) {
        ...
        if (actionMasked != 0) {
            if (actionMasked == 1) {
                ...
                if (actionMasked == 3) {
                    this.mPressed = false;
                    if (this.mMoving) {
                        performAction(Const.Param.NOTIFICATION_CANCEL);
                        resetInner();
                        this.mStartAnimTime = -1L;
                        z2 = false;
                        z3 = true;
                ...
                double d2 = y2;
                double d3 = d2 - this.mTouchStartY;
                this.mOffsetY = d3;
                this.mOffsetX = ((double) x2) - this.mTouchStartX;
            ...
            if (touched(x2, y2)) {
                ...
                int iFloor = (int) Math.floor((y2 - this.mInnerGroup.getAbsoluteTop()) / this.mItem.getHeight());
