...
import com.miui.maml.util.CustomUtils;
...
public class ImageScreenElement extends AnimatedScreenElement implements BitmapProvider.IBitmapHolder {
    ...
    private Expression mRawBlurRadiusExp;
    ...
    private void load(Element element) {
        ...
        Expression expressionBuild = Expression.build(variables, getAttr(element, "blurExp"));
        this.mRawBlurRadiusExp = expressionBuild;
        ...
        this.mDisplayFlags = getAttr(element, "displayFlags");
        if (!this.mRoot.isIconLoad()) {
    ...
    private void renderWithMask(Canvas canvas, Mask mask, int i2, int i3) {
        ...
        if (mask.isAlignAbsolute()) {
            ...
            if (rotation2 == 0.0f) {
                ...
                double dDoubleValue = ((double) i2) + this.mRotateXYpair.f2547p1.doubleValue();
                ...
                double dScale = (x2 + ((double) scale(this.mRotateXYpair.f2547p1.doubleValue()))) - dDoubleValue;
                double dScale2 = (y2 + ((double) scale(this.mRotateXYpair.p2.doubleValue()))) - dDoubleValue2;
                double dSqrt = Math.sqrt((dScale * dScale) + (dScale2 * dScale2));
                double dAsin = Math.asin(dScale / dSqrt);
                double d3 = dScale2 > 0.0d ? d2 + dAsin : (d2 + PI) - dAsin;
                dSin = dSqrt * Math.sin(d3);
                dCos = dSqrt * Math.cos(d3);
                rotation = f2;
            }
            x2 = dSin - ((double) getX());
            y2 = dCos - ((double) getY());
        }
        canvas.rotate(rotation, (float) (((double) mask.getPivotX()) + x2 + ((double) i2)), (float) (((double) mask.getPivotY()) + y2 + ((double) i3)));
        int i4 = (int) x2;
        int i5 = (int) y2;
    ...
    /* JADX WARN: Type inference failed for: r6v1, types: [T1, T2, java.lang.Double] */
    /* JADX WARN: Type inference failed for: r6v2, types: [T1, java.lang.Double] */
    /* JADX WARN: Type inference failed for: r6v4, types: [T2, java.lang.Double] */
    private void rotateXY(double d2, double d3, double d4, pair<Double, Double> pairVar) {
        double dSqrt = Math.sqrt((d2 * d2) + (d3 * d3));
        ?? ValueOf = Double.valueOf(0.0d);
        if (dSqrt <= 0.0d) {
            pairVar.f2547p1 = ValueOf;
            pairVar.p2 = ValueOf;
        } else {
            double dAcos = (PI - Math.acos(d2 / dSqrt)) - d4;
            pairVar.f2547p1 = Double.valueOf(d2 + (Math.cos(dAcos) * dSqrt));
            pairVar.p2 = Double.valueOf(d3 - (dSqrt * Math.sin(dAcos)));
        }
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* JADX INFO: renamed from: setBitmapInner, reason: merged with bridge method [inline-methods] */
    public void lambda$setBitmap$0(Bitmap bitmap) {
    ...
    public void doRender(Canvas canvas) {
        ...
        if (this.mPendingBlur) {
            ...
            this.mBlurBitmap = BitmapFactory.fastBlur(this.mRoot.getContext().mContext, bitmap, this.mBlurBitmap, this.mBlurRadius);
        }
        ...
        if (this.mMasks.size() != 0) {
            ...
            if (ninePatch != null) {
                ...
                MamlLog.e(LOG_TAG, "the image contains ninepatch chunk but couldn't get NinePatch object: " + getSrc());
            }
    ...
    public void doTick(long j2) {
        ...
        if (isVisible()) {
            ...
            Expression expression2 = this.mCornerRadiusXExp;
            if (expression2 != null) {
                this.mCornerRadiusX = scale(expression2.evaluate());
            }
            Expression expression3 = this.mCornerRadiusYExp;
            if (expression3 != null) {
                this.mCornerRadiusY = scale(expression3.evaluate());
            }
            Expression expression4 = this.mRawBlurRadiusExp;
            if (expression4 != null) {
                int iScale = (int) scale(expression4.evaluate());
    ...
    public float getX() {
        float x2 = super.getX();
        SourcesAnimation sourcesAnimation = this.mSources;
        return sourcesAnimation != null ? x2 + scale(sourcesAnimation.getX()) : x2;
    }
    ...
    public float getY() {
        float y2 = super.getY();
        SourcesAnimation sourcesAnimation = this.mSources;
        return sourcesAnimation != null ? y2 + scale(sourcesAnimation.getY()) : y2;
    }
    ...
    public void init() {
        ...
        Expression expression = this.mRawBlurRadiusExp;
        if (expression != null) {
            this.mBlurRadius = (int) scale(expression.evaluate());
        } else {
    ...
    public void loadMesh(Element element) {
        ...
        int iIndexOf = attr.indexOf(aa.f3429b);
        if (iIndexOf != -1) {
            try {
                ...
                MamlLog.w(LOG_TAG, "Invalid mesh format:" + attr);
            }
            ...
            this.mMeshHeight = 0;
            this.mMeshWidth = 0;
            MamlLog.w(LOG_TAG, "Invalid meshVertsArr:" + attr2 + "  undifined or not float[] type");
        }
    }
    ...
    public void setBitmap(final Bitmap bitmap) {
        if (Looper.myLooper() != Looper.getMainLooper()) {
            this.mRoot.getContext().getHandler().post(new Runnable() { // from class: com.miui.maml.elements.b
                @Override // java.lang.Runnable
                public final void run() {
                    this.f2550a.lambda$setBitmap$0(bitmap);
                }
            });
        } else {
            lambda$setBitmap$0(bitmap);
        }
    }
    ...
    public void updateBitmapImpl(boolean z2) {
        ...
        if (bitmap != null && !BitmapProvider.VersionedBitmap.equals(bitmap, this.mCurrentBitmap)) {
            if (bitmap.getBitmap() == null) {
                performAction(MediaRouteProviderProtocol.SERVICE_DATA_ERROR);
            } else {
                performAction("complete");
            }
        }
        if ("wallpaper".equalsIgnoreCase(this.mDisplayFlags) && CustomUtils.isWallpaperImageSupportScale()) {
            Bitmap bitmap2 = bitmap.getBitmap();
            if (bitmap2 != null && this.mWallpaperBitmap == null) {
                if ((bitmap2.getWidth() * 1.0f) / bitmap2.getHeight() > (this.mRoot.getScreenWidth() * 1.0f) / this.mRoot.getScreenHeight()) {
                    this.mWallpaperBitmap = Bitmap.createScaledBitmap(bitmap2, this.mRoot.getScreenWidth(), this.mRoot.getScreenHeight(), false);
                    MamlLog.i(LOG_TAG, "create scale wallpaper image");
                    if (this.mWallpaperBitmap == null) {
                        this.mCurrentBitmap.set(bitmap);
                    } else if (this.mBitmap.getBitmap() != null) {
                        this.mBitmap.setBitmap(this.mWallpaperBitmap);
                        this.mCurrentBitmap.set(this.mBitmap);
                    } else {
                        BitmapProvider bitmapProvider = this.mBitmapProvider;
                        if (bitmapProvider != null) {
                            bitmapProvider.mVersionedBitmap.setBitmap(this.mWallpaperBitmap);
                            this.mCurrentBitmap.set(this.mBitmapProvider.mVersionedBitmap);
                        }
                    }
                } else {
                    this.mCurrentBitmap.set(bitmap);
    ...
    public BitmapProvider.VersionedBitmap getBitmap(boolean z2) {
